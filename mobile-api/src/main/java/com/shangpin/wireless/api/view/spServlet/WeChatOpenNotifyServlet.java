...
public class WeChatOpenNotifyServlet extends BaseServlet {

	private static final long serialVersionUID = -9010714694667515297L;

	protected final Log log = LogFactory.getLog(WeChatOpenNotifyServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!"0".equals(trade_state)) {
			FileUtil.addLog(request, "WeChatOpenNotifyServlet", "channelNo", channelNo, "out_trade_no", out_trade_no, "transaction_id", transaction_id, "total_fee", total_fee,
		...
		try {
			...
			/*if (Constants.WEIXIN_PAYMENT) {
				String payTestUser = Constants.PAY_TEST_USER;
				String[] test = payTestUser.split(",");
				OrderItem testOrder = null;
				for (int i = 0; i < test.length; i++) {
					testOrder = bizOrderService.beOrderDetail(test[i], out_trade_no).getResult();
					log.info("testPay:"+testOrder.getPayamount());
				}
				if (testOrder != null) {
			...
			boolean f = bizOrderService.updateOrderStatus("27", "57", out_trade_no, Constants.PAY_AMOUNT);
