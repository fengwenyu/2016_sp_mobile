 ...
 */
public class NewProductListFilterServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected final Log log = LogFactory.getLog(NewProductListFilterServlet.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (StringUtil.isNotEmpty(gender)) {
			HashMap<String, String> map = FilterSearchUtil.initQueryMap(userlv, userId, brandid, categoryid, null, firstpropid, pricerange, gender, productsize, null, null, null);
			String filterurl = Constants.BASE_SEARCH_URL + "mobile/NewestList";
			try {
				...
				//	保持相同的二级类
				SearchFacetCategoryVO category = NewProductListServlet.getCategory(gender, true);
				searchFacetsVO.setSearchFacetCategoryL2VO(category);
				//品牌用品类控制
				map = FilterSearchUtil.initQueryMap(userlv, userId, null, categoryid, null, null, null, gender, null, null, null, null);
				...
				e.printStackTrace();
				log.error("NewProductListFilterServlet：" + e);
				try {
			...
			// 记录访问日志
			FileUtil.addLog(request, "shangpinnewproductlistfilter", channelNo,
									"gender", gender,
