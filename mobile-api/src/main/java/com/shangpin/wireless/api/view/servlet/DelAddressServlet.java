...
public class DelAddressServlet extends HttpServlet {
	...
	protected final Log log = LogFactory.getLog(DelAddressServlet.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (StringUtil.isNotEmpty(userid, addrid, sessionid, imei)) {
			if (SessionUtil.validate(userid, imei, sessionid)) {
				...
				String url = Constants.BASE_URL_SP + "DelConsigneeAddress/";
				try {
					String data = WebUtil.readContentFromGet(url, map);
					response.getWriter().print(data);
				} catch (Exception e) {
					e.printStackTrace();
					log.error("DelAddressServlet：" + e);
					try {
			...
			// 记录访问日志
			FileUtil.addLog(request, "deladdr", channelNo,
					"userid", userid, 
					"addrid", addrid);
		} else {
			try {
