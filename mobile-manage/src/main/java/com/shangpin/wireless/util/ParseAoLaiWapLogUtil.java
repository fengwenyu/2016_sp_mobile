 ...
 */
public class ParseAoLaiWapLogUtil {
	...
	public static void parseWapLog() {
		try {
			...
			for (int i = 0; i < 31; i++) {
				...
				File file = new File("/home/parselog/wap_aolai_logs/" + name + ".txt");
				if (file.exists()) {
					parseOnedayLog(new File("/home/parselog/wap_aolai_logs/" + name + ".txt"));
				}
			}
	...
	public static void parseYesterdayWapLog() {
		...
		parseOnedayLog(new File("/home/parselog/wap_aolai_logs/" + fileName + ".txt"));
	}
...
//		}
		try {
			...
			AoLaiWapPageViewService pageviewService = (AoLaiWapPageViewService) ac.getBean(AoLaiWapPageViewService.SERVICE_NAME);
			...
			while (logList.size() > 0) {
				...
				if ("alipay/trade".equals(entity.name) || "alipay/clienttrade".equals(entity.name) || "alipay/shangPinAndroidClientTrade".equals(entity.name) || "alipay/webtrade".equals(entity.name)) {
	...
	private static Entity parse(String line) {
		...
		try {
			...
			if ("alipay/webtrade".equals(entity.name) || "alipay/clienttrade".equals(entity.name)) {
				if (!infos[4].startsWith("shopType")) {
					infos[4] = "shopType," + infos[4];
...
//						hqlHelper.addWhereCondition("a.username=?", username);
		...
		public void saveDeviceInfo(DeviceInfoService deviceService) {
			if ("aolaiindex!index".equals(name)) {
				try {
		...
		public void saveOrderStatistics(OrderStatisticsService orderService) {
			if (!"orderStatistics".equals(name)
					&& !"alipay/trade".equals(name)
					&& !"alipay/clienttrade".equals(name)
					&& !"alipay/aoLaiClienttrade".equals(name)
					&& !"alipay/shangPinAndroidClientTrade".equals(name)
					&& !"alipay/webtrade".equals(name)
					&& !"/alipay/notifyReceiver".equals(name)
					&& !"unionpay/trade".equals(name)) {
				return;
			}
			try {
				String city = null;
...
//						order.setOrderType(paramMap.get("ordertype"));
				...
				} else if ("unionpay/trade".equals(name)) {
					long channelNum = 42L;
					final String orderid = paramMap.get("orderid");
					HqlHelper hqlHelper = new HqlHelper(OrderStatistics.class, "a");
					hqlHelper.addWhereCondition("a.orderId=?", orderid);
		...
		public void saveDownloadStatistics(DownloadStatisticsService downloadService) {
			if (!"aolaiindex!toAppStore".equals(name)) {
				return;
			}
			try {
				...
				if (downproduct == null || downproduct.length() <= 0) {
					...
					if ("#".endsWith(downproduct)) return;
				}
				statistics.setDownloadProductNum(Long.parseLong(downproduct));
				statistics.setChannelNum(channelNum);
		...
		public void savePageView(AoLaiWapPageViewService pageviewService) {
			if ("orderStatistics".equals(name)
					|| "/alipay/notifyReceiver".equals(name)
			...
			try {
				AoLaiWapPageView pageview = new AoLaiWapPageView();
