...
public class ManageBaseDaoImpl<T> extends HibernateDaoSupport implements ManageBaseDao<T> {
	...
	public List<T> getListByCondition(HqlHelper hqlHelper) throws Exception {
		...
		if (hqlHelper.getLimitStart() >= 0)
			listQuery.setFirstResult(hqlHelper.getLimitStart());
		if (hqlHelper.getLimitCount() > 0)
			listQuery.setMaxResults(hqlHelper.getLimitCount());
...
//		return sessionFactoryManage.getCurrentSession();
//	}
	// 增加或更新实体
