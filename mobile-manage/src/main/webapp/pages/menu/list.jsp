...
<title>微信公众平台</title>
<link rel="stylesheet" type="text/css" href="${ctx}/style/css/menu/base2968da.css"/>
<link rel="stylesheet" href="${ctx}/style/css/menu/index2968da.css">

<link rel="stylesheet" href="${ctx}/style/css/menu/dialog_img_pick29f4d5.css">
<link rel="stylesheet" href="${ctx}/style/css/menu/emotion_editor23b187.css">
<link rel="stylesheet" href="${ctx}/style/css/menu/msg_tab25df2d.css">
<link rel="stylesheet" href="${ctx}/style/css/menu/msg_sender2968da.css">
...
<div class="main_hd js_menuSetting border"><h2>自定义菜单</h2></div>
<div class="main_bd">
       ...
       <p class="tips_global menu_setting_tips js_menu_setting_tips">可添加最多3个一级菜单，每个一级菜单下可添加最多5个子菜单</p>
        <div class="menu_preview_area">
            <div class="mobile_menu_preview">
                <div class="mobile_hd tc">跑男世诚</div>
                <div class="mobile_bd">
                    <ul class="pre_menu_list grid_line" id="menuList">
                	<c:forEach var="menu" items="${menus.button}">
						<li class="jsMenu pre_menu_item current selected">
						  <a href="javascript:void(0);" class="pre_menu_link" draggable="false">
						    ...
						    <c:if test="${fn:length(menu.sub_button) == 0}">
						    	<c:choose>
						  			<c:when test="${menu.type == 'click'}">
						  				<input type="hidden" name="click" value="${menu.key}"/>
						  			</c:when>
						  			<c:when test="${menu.type == 'view'}">
						  				<input type="hidden" name="view" value="${menu.url}"/>
						  			</c:when>
						  			<c:otherwise>
						  ...
						  <c:choose>
						  	<c:when test="${fn:length(menu.sub_button) > 0}">
							    ...
							    <ul class="sub_pre_menu_list">
							    	<c:forEach var="subMenu" items="${menu.sub_button}">
							    		<li class="jslevel2 current selected" type="${subMenu.type}">
							    			<a href="javascript:void(0);" class="jsSubView" draggable="false">
							    				<span class="sub_pre_menu_inner js_sub_pre_menu_inner">
							    					<c:choose>
							    						<c:when test="${subMenu.type == 'view'}">
							    							<input type="hidden" name="view" value="${subMenu.url}"/>
							    						</c:when>
							    						<c:when test="${subMenu.type == 'click'}">
							    							<input type="hidden" name="click" value="${subMenu.key}"/>
							    						</c:when>
							    					</c:choose>
								    				<i class="icon20_common sort_gray"></i>
								    				<span class="js_l2Title">${subMenu.name}</span>
							    				</span>
							    			</a>
							    		</li>
							    	</c:forEach>
							    	<c:if test="${fn:length(menu.sub_button) < 5}">
							        	<li class="js_addMenuBox"><a href="javascript:void(0);" class="jsSubView js_addL2Btn" title="添加子菜单" draggable="false"><span class="sub_pre_menu_inner js_sub_pre_menu_inner"><i class="icon16_common add_gray"></i></span></a></li>
							    	</c:if>
							    </ul>
					...
					<c:choose>
						<c:when test="${fn:length(menus.button) < 3}">
							<li class="js_addMenuBox pre_menu_item no_extra">
			                    <a href="javascript:void(0);" class="pre_menu_link js_addL1Btn" title="添加菜单" draggable="false">
			                        <i class="icon16_common add_gray"></i>
			                    </a>
			                </li>
						</c:when>
						<c:otherwise>
							<li class="js_addMenuBox pre_menu_item no_extra" style="display: none;">
		                    <a href="javascript:void(0);" class="pre_menu_link js_addL1Btn" title="添加菜单" draggable="false">
			                        <i class="icon16_common add_gray"></i>
			                    </a>
			                </li>
        ...
        <div class="menu_form_area">
            <div id="js_none" class="menu_initial_tips tips_global" style="display: block;">点击左侧菜单进行编辑操作</div>
            <div id="js_rightBox" class="portable_editor to_left" style="display: none;">
                <div class="editor_inner">
                    ...
                    <div class="menu_form_bd" id="view">
                        ...
                        <div class="frm_control_group js_setContentBox" style="display: block;">
                            <label for="" class="frm_label">
                                <strong class="title js_menuContent">菜单内容</strong>
                            </label>
                            <div class="frm_controls frm_vertical_pt">
                                
                                <label class="frm_radio_label js_radio_sendMsg" data-editing="0">
                        ...
                        <div class="menu_content_container" style="display: block;">
                            
                            <div class="menu_content send jsMain" id="edit" style="display: block;">
                                <div class="msg_sender" id="editDiv"><div class="msg_tab">
...
<div class="edit_area js_editorArea" style="display: none;"></div><div id="edit_text" class="edit_area js_editorArea" contenteditable="true" style="overflow-y: auto; overflow-x: hidden;"></div>
    ...
    <div class="img_pick_area">
        <div class="sub_title_bar in_dialog">
            <div class="upload_box r align_right">
                <span class="upload_area webuploader-container"><a id="js_imageupload879262300208211" class="btn btn_primary js_imageupload webuploader-pick" data-groupid="">本地上传<input type="file" accept="image/*" class="upload_file_btn"></a></span>
            </div>
...
<li class="img_item js_imageitem" data-id="401084438" data-url="https://mmbiz.qlogo.cn/mmbiz/DCJnIhSf5C0ESLD4j7ocDia422iahKwic7rcDeffv9P2eibkqxLibnhMBIlStUocny6Ur7kQUBb8lxmKEVfNraT8GkA/0?wx_fmt=jpeg" data-oristatus="0" data-format="jpeg">
<label class="frm_checkbox_label img_item_bd">
    
    <img src="https://mp.weixin.qq.com/cgi-bin/getimgdata?mode=small&source=file&fileId=401084438&token=605032337&lang=zh_CN" alt="img_04.jpg" class="pic">
...
<div class="dialog_wrp delete_dialog_wrp ui-draggable" style="width:720px;margin-left:-360px;margin-top:-185.5px;display:none;">
<div class="dialog" id="wxDialog_9">
<div class="dialog_hd">
  <h3>温馨提示</h3>
...
<div class="dialog_ft">
  ...
  <a href="javascript:;" class="btn btn_primary js_btn" id="delete_btn" onclick="return false;">确定</a>
  
  <a href="javascript:;" class="btn btn_default js_btn" onclick="return false;">取消</a>
  

</div>
</div>
</div>
<div class="mask ui-draggable" style="display:none;"><iframe frameborder="0" style="filter:progid:DXImageTransform.Microsoft.Alpha(opacity:0);position:absolute;top:0px;left:0px;width:100%;height:100%;" src="about:blank"></iframe></div>

<script type="text/html" id="pre_menu_item">
 ...
 <li class="jsMenu pre_menu_item current selected">
  ...
  <div class="sub_pre_menu_box" style="display: block;">
    <ul class="sub_pre_menu_list">
        <li class="js_addMenuBox"><a href="javascript:void(0);" class="jsSubView js_addL2Btn" title="添加子菜单" draggable="false"><span class="sub_pre_menu_inner js_sub_pre_menu_inner"><i class="icon16_common add_gray"></i></span></a></li>
    </ul>
    <i class="arrow arrow_out"></i>
...
<script type="text/html" id="subMenu_menu_item">
<li class="jslevel2 current selected"><a href="javascript:void(0);" class="jsSubView" draggable="false"><span id="add_sub_menu" class="sub_pre_menu_inner js_sub_pre_menu_inner"><i class="icon20_common sort_gray"></i><span class="js_l2Title">子菜单名称</span><input type="hidden"/></span></a></li>
</script>




<script type="text/javascript" src="${ctx}/script/jquery-1.5.min.js"></script>

<script type="text/javascript" src="${ctx}/style/js/menu/menu.js"></script>
