...
<form method="post" action="${contextPath }/management/maintain/staticActivity/update" class="required-validate pageForm" onsubmit="return validateCallback(this, dialogReloadNavTab);">
	<input type="hidden" name="id" value="${staticActivity.id}"/>
	<div class="pageFormContent" layoutH="58">  
		...
		<p>
			<label>开始时间：</label>
			<input type="text" name="startTime" class="date" dateFmt="yyyy-MM-dd HH:mm:ss" readonly="true"  value="${staticActivity.startTime}"/>
			<a class="inputDateButton" href="javascript:;">选择</a>
		</p>
		<p>
			<label>结束时间：</label>
			<input type="text" name="endTime" class="date" dateFmt="yyyy-MM-dd HH:mm:ss" readonly="true" value="${staticActivity.endTime}"/>
			<a class="inputDateButton" href="javascript:;">选择</a>
		</p>
		<p>
			<label>URL跳转链接：</label>
			<input type="text" name="getUrl" value="${staticActivity.getUrl}"/>
			<font color="red"><span>以http://开头</span></font>
		</p>
		<p>
			<label>标题</label>
			<input id="title" name="title" type="text"  value="${staticActivity.title}"/> 
		</p>
		<p>
			<label>文字描述</label>
			<textarea  id="description" name="description"  value="${staticActivity.description}"></textarea>
						
		</p>
		<p>
			<label>客户端类型</label>
			<select name="app">
					<option value="SHANGPIN" ${staticActivity.app.value == 1 ? 'selected="selected"':'' }>尚品</option>
					<option value="AOLAI" ${staticActivity.app.value == 0 ? 'selected="selected"':'' }>奥莱</option>
			</select>
