...
<link href="${contextPath}/styles/uploadify/scripts/uploadify.swf" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="${contextPath}/styles/uploadify/scripts/jquery.uploadify.min.js"></script>
...
<form method="post" action="${contextPath }/management/admin/fashionInfo/create" class="required-validate pageForm" onsubmit="return validateCallback(this, dialogReloadNavTab);" enctype="multipart/form-data">
	<div class="pageFormContent" layoutH="58">
		<p>
			<label>标题：</label>
			<input type="text" name="title" class="validate[required,minSize[1],maxSize[50]] required" size="30" maxlength="51"/>
			<!-- 
		...
		<p>
			<label>作者：</label>
			<input type="text" name="author" class="validate[maxSize[20]]" size="20" maxlength="21" id="author"/>
		</p>
		<p>
			<label>摘要：</label>
			<div>
				<textarea rows="5" cols="50" name="digest" class="validate[maxSize[100]]" maxlength="101"></textarea>
			</div>
		</p>

		<p>
			<label>发布时间：</label>
			<input type="text" name="releaseTime" class="date" readonly="readonly" pattern="yyyy-MM-dd HH:mm" style="float:left;"/>
		...
		<dl class="nowrap">
			<dt>资讯内容：</dt>
			<dd>
				<textarea class="textInput" cols="55" rows="5" name="ckeditor" ></textarea>
				<script type="text/javascript">
