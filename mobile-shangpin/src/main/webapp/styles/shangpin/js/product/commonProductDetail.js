function getSize() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.productSize.sizeContent !=null){
				len3=data.productSize.sizeContent.length; 
			}
			...
			if(len3>0){
				$("#tabs_txt0").append(data.productSize.sizeContent);
			}
			if(groupsLen>0){
			...
			if(len<=0&&len2<=0&&len3<=0&&modelClothPic==null){
...
function getComment(type) {
	...
	if(commentCount*1==0)
	{
		$("#for_comments").children().remove();
		$("#for_comments").append("<div class='no_comment'> <p>暂无评论</p> </div>");
	}
	if(start*3<=(commentCount*1+3)){
		var path = getRootPath();
		$.ajax({
			url : path + "/product/getComment",
			data : {
				"productNo" : $('#productNo').val(),
				"pageIndex" : start,
				"pageSize" : 3
				...
				if (len > 0) {
					$.each(data.productComment.list, function(index, item) {
						...
						if(item.productInfo!=null&&item.productInfo!=''){
							$span1.text("评价商品："+item.productInfo);
						}
						if(item.userInfo!=null&&item.userInfo!=''){
							$span2.text("用户信息："+item.userInfo);
						}
						$p1.append($span1);
						$p1.append($span2);
						$div_cImg.append($p1);
						...
						if (type == 1) {
							$div_concom_h.append($div);
							$div_concom_h.append($div_cmt);
							$div_concom_h.append($div_cImg);
							$div_concom_h.append($div_reply);
							$div_concom_h.attr({
								"style" : "display: block;"
							});
							$(".comment_height").append($div_concom_h);
						} else {
...
$(".comment_tab").find("li").on("click",function(){
	...
	var path = getRootPath();
	window.location.href=path+"/product/prCommentList?productNo="+productNo+"&tagId="+tagId;
})

//评论筛选tab
$(".comment_height").on("click",function(){
	goCommentList();
})
 function goCommentList(){
	...
	var productNo = $("#productNo").val();
	var path = getRootPath();
	window.location.href=path+"/product/prCommentList?productNo="+productNo+"&tagId=0";

}

//点击大图片显示图片的列表
$(".case_img").click(function(){
