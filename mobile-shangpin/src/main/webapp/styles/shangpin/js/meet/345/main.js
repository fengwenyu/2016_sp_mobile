$(document).ready(function(e) {
	...
	$(".show-rule").click(function(){
		...
		$("#rule-box").addClass("rule-box-on");
		$(".section-rule").addClass("section-rule-on");
		return false;
	});
	$(".section-rule").children("a.close-rule").click(function(){
		$("html,body").removeClass("onBody");
		$("#rule-box").removeClass("rule-box-on");
		$(".section-rule").removeClass("section-rule-on");	
		return false;
	})
	$("#rule-box").click(function(e){
		$("html,body").removeClass("onBody");
	  if(e.target.classList.contains('rule-box')){
		$("#rule-box").removeClass("rule-box-on");
		$(".section-rule").removeClass("section-rule-on");		
	  }
	});
...
//		}
  ...
  //点击领取按钮时验证输入内容是否正确
	$(".sure-btn").click(function(){
     ...
     //var time="2015-09-13 21:48:41";
	 var  year = time.substring(0, 10).split('-');
     var  hour = time.substring(11, 19).split(':');
      if(year[2] >12){ $("#region1").show();$("#region2").show();$("#region3").show();}

      if(year[0] ==2015&&year[1] ==09&&year[2] ==12){
