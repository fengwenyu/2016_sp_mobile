...
<div class="paymet_block">
	 <form name="login" id="J_Login">
            <fieldset>
                ...
                <c:forEach var="delivery" items="${elements.delivery}">
                	<c:choose>
                ...
                <c:if test="${elements.postArea=='1'}">
	              <legend>发票信息</legend>
	                ...
	                <p class="select invoice" style="display: none;">
	                	<c:if test="${elements.lastInvoiceFlag == '1'}">
	                		<a href="javascript:showPage('order_invoice','order_invoice_header');">
	                		<c:choose>
	                			<c:when test="${elements.lastInvoiceType == '1' }">
	                			...
	                			<c:otherwise>
	                				发票类型：<em id="invoice_type_text">公司</em><br />
					                        ...
					                        发票抬头：<em id="invoice_title">${elements.lastInvoiceTitle}</em><br />
					                        发票内容：<em id="invoice_content">商品明细</em>
	                 		</a>
	<%--                  		<input type="hidden" id="invoice_addr" name="invoiceaddrid" value="${elements.lastInvoiceConsigneeID}"/> --%>
								<input type="hidden" id="invoice_addr" name="invoiceaddrid" value="${elements.lastInvoiceConsigneeID}"/>
	                	</c:if>
	                    <c:if test="${elements.lastInvoiceFlag != '1'}">
	                		<a href="javascript:showPage('order_invoice','order_invoice_header');">
                ...
                <c:forEach var="spList" items="${elements.list.spList}">
                	<c:forEach var="product" items="${spList.cartItemList}">
                		<p class="pord_info" id="${product.shoppingCartDetailId}">
	                		<c:choose>
		                    ...
		                    <ins>
		                        <i>${product.brandEnName}<br />${product.productName}</i>
                ...
                <p class="select coupons"><a href="javascript:showCouponPage('order_coupons','order_coupons_header');">优惠券/折扣码 <i id="use_conpus">您有 <em>${fn:length(elements.coupon)}</em> 张优惠券可用</i></a></p>
                <input type="hidden" id="totalCoupon" value="${fn:length(elements.coupon)}"/>
                <c:if test="${elements.giftCardBalance != '0'}">
                	<legend>礼品卡支付</legend>
	                <p class="giftCard"><a href="#">余额 &yen;<i id="giftCard">${elements.giftCardBalance}</i></a></p>
	              	<legend class="tips"></legend>
                </c:if>
              <legend>支付方式</legend>
                <p>您还需要支付剩余 &yen;<i id="surplus">${fn:substring(real_pay,0,fn:indexOf(real_pay,'.'))}</i></p>
                <input type="hidden" id="zhifu_codincart" value="${elements.codincart}"/>
                <c:choose>
		                ...
		                <c:choose>
				        	...
				        	<c:otherwise>
				                ...
				                <div class="other-payment-box">
					                ...
					                <c:choose>
					                	<c:when test="${elements.codincart == '1' && haveAddress && address.cod == '1' }">
					                	...
					                	<c:otherwise>
					                		<p style="display: none;" class="payment-method hd_zhifu">
							               	  <span id="zhifu" class="cashPay">
							                    	<a href="javascript:;">货到付款现金支付</a>
							                    </span>
							                </p>
               ...
               <input type="hidden" id="ori_carriage" name="ori_carriage" value="${ori_carriage.amount}"/>
           	   ...
           	   <input type="hidden" id="promoPrice" name="promoPrice" value="${fn:substring(elements.promoAmount,0,fn:indexOf(elements.promoAmount,'.'))}"/>
               ...
               <input type="hidden" id="carriageAmount" value="${fn:substring(elements.carriageAmount,0,fn:indexOf(elements.carriageAmount,'.'))}"/>
              <p class="total">
              	   <c:forEach var="priceShow" items="${elements.priceShow}">
              	   	<c:choose>
              	   		...
              	   		<c:when test="${priceShow.type == '3' && priceShow.amount != '0'}">
              	   			 <span id="span_pay_coupon">优惠：<i id="pay_coupon">&nbsp;&nbsp;-&yen;&nbsp;<b style="font-weight: normal;"></b></i><br /></span>
              	   		</c:when>
                ...
                <div class="payment_submit payment_submit_bg">
					...
					<input type="hidden" id="invoicetitle" name="invoicetitle" value="${elements.lastInvoiceTitle}"/>
					...
					<input type="hidden" name="express" id="delivery"" value="${elements.lastDeliveryType}"/>
					...
					<c:choose>
						<c:when test="${elements.postArea=='2' }">
							<input type="hidden" id="postArea" name="postArea" value="2"/>
						</c:when>
						<c:otherwise>
					...
					<div class="btn-icon clr">
                    	...
                    	<a id="submit_order" href="javascript:submit();" class="payment_btn">提交订单（${elements.totalSettlement}件）</a>
                    </div>
                </div>
            </fieldset>
        </form>
