			...
			<footer>
				...
				<c:choose>
					<c:when test="${cart.totalAmount != '0' }">
						<a id="immediately" class="btn immediately" href="javascript:;">结算(<em id="total_number">${cart.totalSettlement}</em>)
						</a>
					</c:when>
					<c:otherwise>
						<a class="btn immediately no_submit" href="javascript:;">结算(<em
							id="total_number">0</em>)
						</a>
					</c:otherwise>
				</c:choose>
