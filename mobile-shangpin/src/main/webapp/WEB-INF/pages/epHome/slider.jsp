...
<%@ include file="/WEB-INF/pages/common/include.inc.jsp"%>
 <p><img alt="" src="${cdn:pic(pageContext.request)}/styles/shangpin/images/ep/ep_tip1.jpg" width="100%"></p>
 	 				...
 	 				<c:choose>
 	 					<c:when test="${gallery.type == '1'}">
	 						<c:choose>
				 				...
				 				<c:otherwise>
				 					<a href="${ctx}/subject/product/list?topicId=${gallery.refContent}&postArea=2">
				 				</c:otherwise>
	 						</c:choose>
	 					</c:when>
	 					<c:when test="${gallery.type == '3'}">
		 					<c:choose>
				 				...
				 				<c:otherwise>
				 					<a href="${ctx}/category/product/list?categoryNo=${gallery.refContent}&postArea=2">
				 				</c:otherwise>
		 					</c:choose>				
	 					</c:when>
	 					<c:when test="${gallery.type == '4'}">
	 				 		<c:choose>
				 				...
				 				<c:otherwise>
				 					 <a href="${ctx}/brand/product/list?brandNo=${gallery.refContent}&postArea=2">
				 				</c:otherwise>
		 					</c:choose>
	 					...
	 					<c:otherwise>
 	 					...
 	 					<img alt="${gallery.name}" src="${fn:substring(gallery.pic,0,fn:indexOf(gallery.pic,'-'))}-479-202.jpg" width="100%"></a> 
 	 			</div>
 	 		</c:forEach>
 	 	</div>
