   ...
   <c:otherwise>
	...
	<c:choose>
		...
		<c:when test="${list.type=='100' ||list.type=='101'}">
			<img alt="" src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg" /></a>
		</c:when>
		<c:otherwise>
			<img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg"></a>
			<c:if test="${list.type!='1'&& list.type!='100'&& list.type!='101'}">
			 </li>
			</c:if>
		</c:otherwise>
