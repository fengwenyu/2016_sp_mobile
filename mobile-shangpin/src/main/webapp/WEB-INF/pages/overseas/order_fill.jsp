...
<rapid:override name="content">
	<div class="paymet_block">
	            <fieldset>
	              ...
	              <div id="showToAddr" <c:if test="${defAddress!=null}">style="display:none;"</c:if>>
	                <legend>收货信息</legend>
	                <p class="select"><a href="javascript:;" id="to_add_address">填写配送地址 </a> </p>
	              </div>
	              	<c:if test="${defAddress==null}">
	              		<input type="hidden" id="third_id" value=""/>
                		<input type="hidden" id="fourth_id" value=""/>
	              	</c:if>
	                <div id="showAddr" <c:if test="${defAddress==null}">style="display:none;"</c:if>>
		                	<legend>配送地址</legend>
			                <p class="selected"><a href="javascript:;" id="to_list_address"><em>${defAddress.name }</em><i class="phone">${defAddress.tel }</i><br>
			                <input type="hidden" id="third_id" value="${defAddress.area}"/>
                			<input type="hidden" id="fourth_id" value="${defAddress.town}"/>
			                	 <em id="emcardID" 
			                	 <c:if test="${defAddress.cardID==null ||  defAddress.cardID==''}"> style="display:none;"</c:if>
			                	 >${fn:substring(defAddress.cardID,0,6)}***${fn:substring(defAddress.cardID,fn:length(defAddress.cardID)-4,fn:length(defAddress.cardID)) }</em><br id="embr"
			                	  <c:if test="${defAddress.cardID==null ||  defAddress.cardID==''}"> style="display:none;"</c:if>
			                	 />
			              
			                <span>${defAddress.provName }${defAddress.cityName }${defAddress.areaName }${defAddress.townName}${defAddress.addr}</span></a></p>
	                 </div>
	                <p class="select"><a href="javascript:;" id="select_time">配送方式<span class="delivery_mode">工作日收货</span></a></p>
	                <legend>商品信息</legend>
	                <c:forEach var="product" items="${fillData.list }">
		                <p class="pord_info">
		                    ...
		                    <ins>
		                        ...
		                        ${product.brandNameEN}<br> ${product.name }</i>
		                        <c:forEach var="pp" items="${product.attribute}">
		                        	 <em>${pp.name }：${pp.value }</em>
		                        </c:forEach>
		                        </br>
	                ...
	                <input type="hidden" id="totalAmount" value="${fillData.totalAmount}"/>
	                <input type="hidden" id="buyId" value="${fillData.buyId}"/>
	                <input type="hidden" id="totalCoupon" value="${fn:length(fillData.coupon)}"/>
	                <p class="select coupons"><a href="javascript:showCouponPage();">优惠券 <i id="use_conpus">您有 <em>${fn:length(fillData.coupon)}</em> 张优惠券可用</i></a></p>
	                <legend>支付方式</legend>
	                <p id="last_pay">您还需要支付剩余 &yen;${fillData.payAmount }</p>
	                <c:choose>
			                	...
			                	<c:otherwise>
			                		...
			                		<p class="payment-method">
					                	<span class="alipayOut">
					                    	<a href="javascript:;" class="cur" mainId="">
					                            <em>支付宝支付</em><br />
					                            储蓄卡支付需开通网银
					                        </a>
					                    </span>
	              ...
	              <p class="total">
	              	<c:forEach var="priceShow" items="${fillData.priceShow}">
		              	<c:choose>
...
<%@ include file="order_fill_invoice.jsp" %> 
<%@ include file="order_fill_invoice_address.jsp" %> 
<%@ include file="order_fill_invoice_address_add.jsp" %> 
<%@ include file="order_fill_invoice_address_edit.jsp" %> 
