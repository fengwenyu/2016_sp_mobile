 ...
 <div class="detailed_information">
 	<div class="product_show">
        <div class="show_case">
              ...
              <a href="javascript:;" class="photo_details"><img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:replace(productDetail.basic.allPics[0],'-10-10','-600-758') }" /></a>
        ...
        <section>
              ...
              <c:choose>
                <c:when test="${productDetail.notice.salesPromotionNew.type  ne null }">
                 <c:set var="model" value="${productDetail.notice.salesPromotionNew}"/>
	               ...
	               <c:otherwise>
	                   <%@ include file="/WEB-INF/pages/ep_product/model_rule.jsp"%>
                       <p> ${model.name } </p>
	                   </a>
	               </c:otherwise>
	             </c:choose>
          ...
          <div>
           ...
           <c:set var="color" value="${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[0].color eq ''? '#000' : productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[0].color}"/>
              <p style='color:${color}'>${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[0].priceTitle}
              <c:choose>
				<c:when test="${productDetail.basic.firstProps[firstP].secondProps[secondP].isPromotion=='0'}">
				 	<em style='color:${color}' id="forSalsPrice not_promotion">
						<c:choose>
				...
				<pdesc style="color: #888;">&nbsp;&nbsp;${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[0].priceDesc}</pdesc>
              </p>
            </div>
            <c:if test="${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[1].isShow ==1}">
			  <c:set var="colorTy" value="${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[1].color eq ''? '#888' : productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[1].color}"/>
			  <div style="color: ${colorTy}" class="font_gray">
              <ol>
                <li>${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[1].priceTitle}${productDetail.basic.firstProps[firstP].secondProps[secondP].shopPrice }&nbsp;&nbsp;${productDetail.basic.firstProps[firstP].secondProps[secondP].priceTag[1].priceDesc}</li>
              </ol>
             </div>
			 </c:if>
          </article> 
