...
public class Trade extends HttpServlet {
	...
	private static Log logInfo = LogFactory.getLog(Trade.class);
	//支付宝回调地址修改为线上外网环境
	String basePath = Constants.AOLAI_URL;
	//支付宝回调地址修改为线上外网环境
	String callbackUrl = Constants.ALIPAY_CALLBACK_URL;
...
//		String subpaymode = request.getParameter("subpaymode");
		...
		balanceServerResponse.json2Obj(json);
		// 记录访问日志
	...
	private Map<String, String> prepareTradeRequestParamsMap(HttpServletRequest request) throws IOException {
		...
		String merchantUrl = basePath+"payokaction!result?parmresult="+parmresult;
		// req_data的内容
