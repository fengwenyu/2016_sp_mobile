...
import com.shangpin.mobileAolai.platform.vo.BalanceServerResponse;
...
public class ClientTrade extends HttpServlet {
	...
	private static Log logInfo = LogFactory.getLog(ClientTrade.class);
	//支付宝回调地址修改为线上外网环境
	//String basePath = "http://m.aolai.com";
	//支付宝回调地址修改为线上外网环境
	String callbackUrl = "";
	String interfaceName = "";
	String productNum = "";
	//修改为尚品客户端指定地址
	String spCallBackUrl = Constants.AOLAI_URL+"alipay/shangpinclientcallBack";
	String alCallBackUrl = Constants.AOLAI_URL+"alipay/aolaiclientcallBack";
	String spAndroidCallBackUrl = Constants.AOLAI_URL+"alipay/shangpinandroidclientcallBack";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		productNum = request.getHeader("p");
		if (productNum == null || "".equals(productNum)) {
			productNum = request.getParameter("p");
		}
		logInfo.debug("ClientTrade shopType is :" + shopType + " ; orderId is :" + orderId + " ; userId is :" + userid + " ; productNum is :" + productNum);
		if ("2".equals(productNum)) {
			interfaceName = "alipay/clienttrade"; // 尚品ios客户端
			callbackUrl = spCallBackUrl;
		} else if ("1".equals(productNum)) {
			interfaceName = "alipay/aoLaiClienttrade"; // 奥莱ios客户端
			callbackUrl = alCallBackUrl;
		} else if ("102".equals(productNum)) {
			interfaceName = "alipay/shangPinAndroidClientTrade";// 尚品android客户端
			callbackUrl = spAndroidCallBackUrl;
		} else if ("101".equals(productNum)) {
			interfaceName = "alipay/clienttrade"; // 奥莱android客户端
			callbackUrl = spCallBackUrl;
		} else {
			interfaceName = "alipay/clienttrade"; // 默认
			callbackUrl = spCallBackUrl;
		}
...
//		map.put("shoptype", shopType);
		...
		// 记录访问日志
		FileUtil.addLog(request, interfaceName,//
				"32",//
				"shopType", shopType,//
				"userid", userid,//
				"orderid", orderId, "mainpaymode", mainpaymode, "subpaymode", subpaymode, "ordertime", balanceServerResponse.getDate(), "orderamount", balanceServerResponse.getAmount(), "orderstatus", balanceServerResponse.getStatus(), "orderstatusdesc", balanceServerResponse.getStatusdesc(), "orderOrigin", balanceServerResponse.getOrderOrigin(), "orderOriginDesc", balanceServerResponse.getOrderOriginDesc(),
				// "data", apiResponse.getData(),
				"cancancel", balanceServerResponse.getCancancel(), "canpay", balanceServerResponse.getCanpay(), "canconfirm", balanceServerResponse.getCanconfirm());
	...
	private Map<String, String> prepareTradeRequestParamsMap(HttpServletRequest request, String totalFee) throws IOException {
		...
		String merchantUrl = "";
		if ("2".equals(productNum)) {
			merchantUrl = "ShangPinAPP://AlixPayWeb/fail"; // 尚品ios客户端
		} else if ("1".equals(productNum)) {
			merchantUrl = "ShangPinAolai://AlixPayWeb/fail"; // 奥莱ios客户端
		} else if ("102".equals(productNum)) {
			merchantUrl = "ShangPinAPP://AlixPayWeb/fail";// 尚品android客户端
		} else {
			merchantUrl = "ShangPinAPP://AlixPayWeb/fail"; // 尚品ios客户端
		}
		// req_data的内容
