 ...
 */
public class AoLaiClientCallBack extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!verified || !result.equals("success")) {
				response.sendRedirect("ShangPinAolai://AlixPayWeb/fail");
				// 记录访问日志
				FileUtil.addLog(request, 
						"/alipay/aolaiclientcallBack",
						channelNo,
						"code", "fail",
						"tradeNo", tradeNo);

			} else {
				response.sendRedirect("ShangPinAolai://AlixPayWeb/sucess");
				// 记录访问日志
				FileUtil.addLog(request, "/alipay/aolaiclientcallBack",
						channelNo,
						"code", "success",
						"tradeNo", tradeNo);
