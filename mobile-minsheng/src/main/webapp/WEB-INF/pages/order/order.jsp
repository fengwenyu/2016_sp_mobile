...
<script type="text/javascript" charset="utf-8">
	...
	function confiradd(){
			if(browser.versions.android==true){				
				window.MsgJs.setAlertInfo('{"title":"提示","msg":"确认保存该地址吗？","ok_btn":"true","ok_text":"确定","ok_func":"addInvoiceajax();","cancle_text":"取消","cancle_func":""}');
			}else if(browser.versions.iPhone==true||browser.versions.iPad==true||browser.versions.ios==true){
		...
		function iphonealertfir(){
			var s='{"title":"提示","msg":"确认保存该地址吗？","ok_btn":"true","ok_text":"确定","ok_func":"addInvoiceajax()","cancle_text":"取消","cancle_func":""}';
		return s;
		}
    ...
    function addAddress(){
    	...
    	if(undefined==validate){
    		$.ajax({
    			  ...
    			  data:{"consigneeAddressVO.name":encodeURI($("#J_AddrName").val()),
			  ...
			  "consigneeAddressVO.town":$("#J_AddrTown").val(),
    			  ...
    			  "consigneeAddressVO.addr":encodeURI($("#J_AddrText").val()),
    			  ...
    			  success:function(data){
    			    if(data.code==0){
    			      ...
    			      var append = '<ul class="address_list">'
    			        + '<li class="li_addrList ' + (obj.isd=='1'?'curr"':'"') + 'id="'+obj.id+'" uname="'+obj.name+'" addr="'+obj.addr+'" tel="'+obj.tel+'" >'
    			        + '<section class="alOrder_addr addr_list" style="background:none">'
    			        + '<a href="javascript:void(0);" id="'+obj.id+'" uname="'+obj.name+'" addr="'+obj.addr+'" tel="'+obj.tel+'" onclick="setAddress(this);window.history.go(-1);">'
    			        + '<dl>'
    ...
    function addInvoiceajax(){
		$.ajax({
			  url:'accountaction!ajaxsaveinvoiceaddress?ch=${ch}',
			  data:{"consigneeAddressVO.name":encodeURI($("#INV_AddrName").val()),
			  ...
			  "consigneeAddressVO.addr":encodeURI($("#INV_AddrText").val())
			  },
			  dataType:'json',
	          timeout: 30000,
          error: function (xmlHttpRequest, error) {
			  ...
			  success:function(data){
			    if(data.code==0){
			      ...
			      var append = '<ul class="invaddr_list">'
			        ...
			        + '</dl>'
			        + '<dl>'
			        + '<dt>地　址：</dt><dd>'+obj.provname+obj.cityname+obj.areaname+obj.addr+'</dd>'
			        + '</dl>'
			        + '<dl>'
			        + '<dt>手机号：</dt><dd>'+obj.tel+'</dd>'
			        + '</dl>'
    ...
    function showCoupon(){
    	...
    	if($("#isUseCoupon").val()!="0"){
    		$(".select_coupon").each(function(){
        		if($(this).attr("isUse")==0 ){
    ...
    function selectCoupon(v){
        ...
        if($(v).attr("isUse")==0  ){//点击使用
			...
			isUse=1;
	        // 显示所选优惠券规则
	        showRule="已优惠:&yen;"+$(v).attr("couponAmount")*1;
    ...
    function save(){
    	...
    	$.ajax({
 			  ...
 			  "orderVO.invoicetitle":encodeURI($("#_invoicetitle").val()),
 			  "orderVO.invoicecontent":encodeURI($("#_classVal").val()),
			 ...
			 "orderVO.couponflag":couponflag
              ...
              error: function (xmlHttpRequest, error) {
              	if(error == "parsererror"){
            		alert("用户帐号异常，请重新登录");
            		var enurl=escape("http://m.shangpin.com/accountaction!applogin");
            		window.location.href="<%=path%>/mobileshangpin/accountaction!loginuicallback="+enurl;
            	}else{
            		location.href = "<%=path%>/orderaction!fail?";
            	}
                return;
              },
 			  success:function(data){
 				  if("success" == data.msgcode){
 					 location.href = "<%=path%>/orderaction!success?orderid="+data.orderid+"&amount="+data.amount+"&paytypeid="+data.paytypeid;
 					 return;  
 				  }
 				 if("notifysuccess" == data.msgcode){
 				  ...
 				  if("cart" == data.msgcode){
 				    location.href = "<%=path%>/allcartaction!listCart";
 					 ...
 					 //location.href = "<%=path%>/spindex!index";
 				  }
 			  }
         });
    }
...
<nav class="alNav">
  ...
  <ul>
    ...
    <li><a href="<%=path%>/cartaction!showcart">购物袋</a></li>
    <li>订单</li>
  </ul>
</nav>
 -->
<%String ua =request.getHeader("user-agent").toLowerCase();%>
<div class="alContent" style="padding-bottom:20px;">
    ...
    <div id="addrList">
      <s:iterator value="consigneeAddressList" id="consigneeAddress" status="st">
        <ul class="address_list">
          <li id="${consigneeAddress.id }" cod="${consigneeAddress.cod }" isd="${consigneeAddress.isd }" class="li_addrList" uname="${consigneeAddress.name }" addr="${consigneeAddress.provname}${consigneeAddress.cityname }${consigneeAddress.areaname }${consigneeAddress.addr }" tel="${consigneeAddress.tel }">
              <section class="alOrder_addr addr_list" style="background:none">
                <a href="javascript:void(0);" id="${consigneeAddress.id }" cod="${consigneeAddress.cod }" uname="${consigneeAddress.name }" addr="${consigneeAddress.provname}${consigneeAddress.cityname }${consigneeAddress.areaname }${consigneeAddress.addr }" tel="${consigneeAddress.tel }" onclick="setAddress(this);window.history.go(-1);">
                  <dl>
                    <dt>收货人：</dt><dd>${consigneeAddress.name }</dd>
                  </dl>
                  <dl>
                    <dt>地　址：</dt><dd>${consigneeAddress.provname}${consigneeAddress.cityname }${consigneeAddress.areaname }${consigneeAddress.addr }</dd>
                  </dl>
                  <dl>
...
<!-- 收货人地址显示区域 开始 -->
  <section class="alOrder_addr"  id="alOrder_addr">
    <a href="#app" onclick="javascript:showAddress();">
      ...
      <div id="noneAddr" style="text-align: center;">点击添加收货人地址</div>
      </s:if>
    </a>
  </section>
<!-- 收货人地址显示区域 结束 -->
  ...
  <s:iterator value="deliveryList" id="deliveryVO" status="st">
    <s:if test="#deliveryVO.enable==1">
      <li class="deliverytag <s:if test='#st.index==0'>cur</s:if>" data-title="${deliveryVO.id }"><a href="#">${deliveryVO.name }<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></a></li>
    </s:if>
  </s:iterator>
  </ul>
...
<%--   <h2 class="alOrder_title">支付方式</h2>
  ...
  <s:iterator value="payList" id="payVO" status="st">
      ...
      <li id="al${payVO.id }" data-title="${payVO.id }" class="paytag <s:if test='#st.index==0'>cur</s:if>" ><a href="javascript:void();">${payVO.name }<i><img src="<%=path%>/images/cur_icon.png" width="12" height="12"></i></a></li>
...
<!-- 发票信息 开始 -->
  ...
  <ul class="invoice_select" id="invoice_option">
    <li>
      <dl>
        <dt>发票信息：</dt>
        <dd><a href="#" class="cur" id="invoice_close_a" data-title="0" '><em>不要发票<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a><a href="#" id="invoice_open_a" data-title="1" ><em>要发票<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a></dd>
      </dl>
    </li>
  </ul>
  ...
  <div class="invoice_content">
    ...
    <ul class="invoice_select" id="invoice_title">
      <li>
        <dl>
          <dt>发票抬头：</dt>
          <dd><a href="#" id="invoice_org_a" class="cur" data-title="0"><em>单位<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a><a href="#" id="invoice_person_a" data-title="1"><em>个人<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a></dd>
        </dl>
      </li>
      <span>
    ...
    <ul class="invoice_select" id="invoice_addr">
      <li>
        <dl>
          <dt>发票邮至：</dt>
          <dd><a href="#" class="big cur" data-title="0" onclick='$(".invaddr_list_li").removeClass("curr");$(".addr_errorMsg").hide()'><em>与收货地址相同<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a>
          <s:if test='invoiceaddrsList == null || invoiceaddrsList.size < 10'>
          </s:if>
          <a href="#" data-title="1" id="other_addr"><em>其他地址<i><img src="<%=path%>/images/cur_icon.png"  width="12" height="12"></i></em></a>
          </dd>
        </dl>
          ...
          <li id="${invoiceaddrs.id }" class="invaddr_list_li">
              <section class="alOrder_invaddr" style="background:none">
                <a href="javascript:void(0);" onclick="setInvAddress(${invoiceaddrs.id });">
                  ...
                  <dl>
                    <dt>地　址：</dt><dd>${invoiceaddrs.provname}${invoiceaddrs.cityname }${invoiceaddrs.areaname }${invoiceaddrs.addr }</dd>
                  </dl>
                  <dl>
 ...
 <span>街道：</span>
  ...
  <div id="ms" class="inv_addr_errorMsg addr_errorMsg" style="padding: 0;color: #B51111;text-align: center;line-height: 25px;" ></div>
    ...
    <s:iterator value="couponList" id="couponVO" status="st">
    	...
    	<dd>
            ...
            <ul>
                ...
                <li>
                <a href="#" onclick="javascript:selectCoupon(this);" couponNo="${couponVO.couponno}" couponRule="${couponVO.name }" couponAmount="${couponVO.amount }" class="select_coupon" id ="usecode"  isUse="0">使用</a>
                </li>
            </ul>
        </dd>
      </dl>
...
<div> 
<input type="button" onclick="javascript:getCouponCodeInfo();" id="select_couponcode" useflag="0" value="使用" /><input type="button" onclick="javascript:back();" id="back" value="返回" /></div>
</div>
<!--优惠码信息 结束 --> 
  ...
  <section class="alOrder_list">
    <s:iterator value="productsPromotionVOList" id="productsPromotionVO" >
      ...
      <input type="hidden"  name="shopid" value="<s:property value='#productCartVO.shopDetailId'/>"/>
  ...
  <%-- <form name="orderForm" id="orderForm" action="<%=path%>/orderaction!saveorder"> --%>
    ...
    <input type="hidden" id="ordertype" value="1" name="orderVO.ordertype">
    ...
    <input type="hidden" id="isUseCoupon" value="0" >
  ...
  <%-- </form> --%>
  <a href='javascript:save();' class="alOrder_buyBtn alOrder_submitBtn">提交订单</a>
</div>
