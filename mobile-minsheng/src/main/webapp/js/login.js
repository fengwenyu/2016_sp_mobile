...
function() {
    ...
    $.extend(window.Login.prototype, {
        ...
        editPwdForm: function() {
            $("#J_EditPwdForm").on("submit",
            function(e) {				
				var pwdLen = $("#J_OldPwd").val().length;
				if ($.trim($("#J_OldPwd").val()) == "" || pwdLen < 6){
					return $(".login_errorMsg").html("请输入6-20位密码！"),
					$("#J_OldPwd").parent("p").addClass("error"),
					!1;
				}else{
					$(".login_errorMsg").html("");
					$("#J_OldPwd").parent("p").removeClass("error");
				}
				var newPwdLen = $("#J_NewPwd").val().length;
				if ($.trim($("#J_NewPwd").val()) == "" || newPwdLen < 6){
					return $(".login_errorMsg").html("请输入6-20位密码！"),
					$("#J_NewPwd").parent("p").addClass("error"),
					!1;
				}else{
					$(".login_errorMsg").html("");
					$("#J_NewPwd").parent("p").removeClass("error");
				}
				if ($.trim($("#J_SedPwd").val()) == "" || $.trim($("#J_NewPwd").val()) !== $.trim($("#J_SedPwd").val())){
					return $(".login_errorMsg").html("两次输入密码不一致！"),
					$("#J_rePwd").parent("p").addClass("error"),
					!1;
				}else{
					$(".login_errorMsg").html("");
					$("#J_SedPwd").parent("p").removeClass("error");
				}
            }),
            new $.search(".c-form-search");
        }
