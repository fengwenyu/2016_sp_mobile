...
public class CookieUtil {
	...
	/**
	 * Cookie值格式为xxxx&yyyy，xxxx表示推广参数或合作渠道号，yyyy为时间戳 ，比较2个Cookie值，返回最近保存的Cookie信息
	 * 
	 * @param request
	 *            HttpServletRequest
	...
	public static Map<String, String> getCookieValue(HttpServletRequest request) {
		...
		if (null != cookieMap && cookieMap.size() > 0) {
			Cookie cc = cookieMap.get(Constants.SP_COOKIE_NAME_COOPERATION);
			...
			// 推广1st、合作属于同级别且最高级别，cookie中同时存在时，需要判断最后生成时间
			if (null != cc) {
				if (null != cp1st) {
					String tmp1 = StringUtils.split(cc.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[1];
					String tmp2 = StringUtils.split(cp1st.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[1];
					if (Long.valueOf(tmp1) < Long.valueOf(tmp2)) {
				...
				map.put("source", StringUtils.split(cc.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[0]);// 渠道号
				map.put("st", StringUtils.split(cc.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[1]);
				map.put("type", "2");
				return map;
			} else {
				if (null != cp1st) {
					map.put("c", StringUtils.split(cp1st.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[0]);
					map.put("st", StringUtils.split(cp1st.getValue(), Constants.SP_COOKIE_SEPARATOR_CHAR)[1]);
					map.put("type", "1");
					return map;
				}
				if (null != cp2nd) {
