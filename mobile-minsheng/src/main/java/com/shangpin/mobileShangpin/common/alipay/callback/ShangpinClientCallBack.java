 ...
 */
public class ShangpinClientCallBack extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!verified || !result.equals("success")) {
				...
				response.sendRedirect("ShangPinAPP://AlixPayWeb/fail");
				System.out.println("Illegal sign!!");
				// 记录访问日志
				FileUtil.addLog(request, "/alipay/shangpinclientcallBack","success", "0");
			} else {
				// TODO 修改为主站提供的支付成功跳转地址
				response.sendRedirect("ShangPinAPP://AlixPayWeb/sucess");
				System.out.println("RSA signature verification success!!");
				// 记录访问日志
				FileUtil.addLog(request, "/alipay/shangpinclientcallBack","success", "1");
			}
