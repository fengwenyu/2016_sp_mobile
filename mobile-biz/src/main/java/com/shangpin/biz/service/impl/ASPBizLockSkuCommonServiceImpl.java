...
@Service
public class ASPBizLockSkuCommonServiceImpl implements
		ASPBizLockSkuCommonService {
	public static Logger log = LoggerFactory.getLogger(ASPBizLockSkuCommonServiceImpl.class);
	@Autowired
	ASPBizOrderService aspBizOrderService;
	@Autowired
	ASPBizLockSkuService aspBizLockSkuService;
	@Override
	public Map<String, String> orderLockSku(String orderNo,String userid,String mainOrderid) {
		...
		return returnMap;
	/*	//1:查询锁库存接口
		List<SupplierSkuNoInfo> list=aspBizLockSkuService.getOrderLockSupplierSkuList(orderNo);
	...
	private Map<String, String> baseOrderLockSku(String orderNo,String userid,String mainOrderid,List<SupplierSkuNoInfo> list){
		...
		for(int i=0;i<list.size();i++){
			...
			ResultContent resultContent=aspBizLockSkuService.lockSku(list.get(i).getSupplierNo(), orderNo, orderItemList);
		...
		if(map.get("code").equals("1")){
			ResultBase resultBase=aspBizOrderService.beCancelOrder(userid, mainOrderid);
			...
			for (Map.Entry<String, String> entry : map.entrySet()) {   
				...
				ResponseResult responseResult=aspBizLockSkuService.stockAbnormalSyncZero(keyArr[0], keyArr[1], orderNo, userid, entry.getValue());
				String resposeStatus=responseResult.getResposeStatus();
				if(!resposeStatus.equals(Constants.STOCKSYNCZERO_SUCCESS)){
					ResponseResult result=aspBizLockSkuService.stockAbnormalSyncZero(keyArr[0], keyArr[1], orderNo, userid, entry.getValue());
