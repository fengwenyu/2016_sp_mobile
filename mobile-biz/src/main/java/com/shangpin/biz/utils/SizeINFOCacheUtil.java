...
public class SizeINFOCacheUtil {
	...
	private static Cache htmlInfoCache;
	static {
		...
		htmlInfoCache = cache.getCache("htmlInfo_cache");
	...
	public static void putHtml(Serializable key, Serializable value) {
		Element element = new Element(key, value);
		if(null != htmlInfoCache){
			htmlInfoCache.put(element);
		} else {
			log.warn("FileuploadTerminalServlet：" + " load EHCache failure!!!");
		}
	}
	...
	public static String getHtmlValue(Serializable key) {
		if (null == htmlInfoCache) {
			log.warn("FileuploadTerminalServlet：" + " load EHCache failure!!!");
			return null;
		}
		
		Element result = htmlInfoCache.get(key);
		if (null != result) {
			Object obj = result.getObjectValue();
			if (null != obj) {
				return obj.toString();
